...
import { ArrowLeft, Trash2, ArrowUp, ArrowDown, X, AlertCircle, UserPlus, FileText } from 'lucide-react';
...
const mockProgram = {
  ...
  capacity: 30,
  enrolled: [
    { id: 1001, name: "John Smith", residentId: "R-1001", enrolledDate: "2024-09-15" },
    { id: 1002, name: "Maria Garcia", residentId: "R-1002", enrolledDate: "2024-09-18" },
    { id: 1003, name: "David Lee", residentId: "R-1003", enrolledDate: "2024-09-22" },
    { id: 1004, name: "Sarah Johnson", residentId: "R-1004", enrolledDate: "2024-10-01" },
    { id: 1005, name: "Michael Brown", residentId: "R-1005", enrolledDate: "2024-10-02" },
  ],
  waitlist: [
    { id: 2001, name: "James Wilson", residentId: "R-2001", requestedDate: "2024-09-25", notes: "Completed prerequisite math course" },
    { id: 2002, name: "Linda Martinez", residentId: "R-2002", requestedDate: "2024-09-28", notes: "" },
    { id: 2003, name: "Robert Taylor", residentId: "R-2003", requestedDate: "2024-10-01", notes: "Release date in 8 months" },
    { id: 2004, name: "Patricia Anderson", residentId: "R-2004", requestedDate: "2024-10-02", notes: "" },
  ],
  kites: [
    { 
      id: 3001, 
...
function ProgramDetailPage() {
  ...
  const shouldShowAlert = spotsAvailable > 0 && hasWaitlist && !dismissedAlerts.includes('spot_available');
  ...
  const handleRemoveFromWaitlist = (residentId) => {
    setProgram(prev => ({
      ...prev,
      waitlist: prev.waitlist.filter(resident => resident.id !== residentId)
    }));
  };
  ...
  const handleMoveUp = (index) => {
    ...
    setProgram(prev => {
      ...
      [newWaitlist[index - 1], newWaitlist[index]] = [newWaitlist[index], newWaitlist[index - 1]];
  ...
  const handleRemoveFromEnrolled = (studentId) => {
    setProgram(prev => ({
      ...prev,
      enrolled: prev.enrolled.filter(student => student.id !== studentId)
    }));
  };
  ...
  const handleEnrollFromWaitlist = (resident) => {
    ...
    const updatedWaitlist = program.waitlist.filter(r => r.id !== resident.id);
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              <div className="text-lg font-semibold text-gray-700">
                Capacity: <span className="text-blue-600">{enrollmentCount}/{capacity}</span>
              </div>
              <button
        ...
        {showAuditLog && (
          <div className="bg-white rounded-lg shadow border border-gray-200 mt-8">
            <div className="px-6 py-4 border-b border-gray-200 bg-gray-50">
              ...
              <p className="text-sm text-gray-600 mt-1">Complete history of all actions for this program</p>
            </div>
        ...
        {shouldShowAlert && (
          <div className="mb-6 bg-green-50 border-l-4 border-green-500 rounded-lg p-4 flex items-start justify-between">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="font-semibold text-green-900">Spot Available</h3>
                <p className="text-sm text-green-800 mt-1">
                  There {spotsAvailable === 1 ? 'is' : 'are'} {spotsAvailable} open {spotsAvailable === 1 ? 'spot' : 'spots'} in this program with {program.waitlist.length} {program.waitlist.length === 1 ? 'person' : 'people'} on the waitlist.
                </p>
              </div>
            </div>
            <button
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-white rounded-lg shadow">
              <div className="px-6 py-4 border-b border-gray-200">
                <h2 className="text-xl font-semibold text-gray-900">Currently Enrolled</h2>
                <p className="text-sm text-gray-500 mt-1">{enrollmentCount} students</p>
              </div>
                    ...
                    {program.enrolled.map((student) => (
                      <tr key={student.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm">
                          <button
                            onClick={() => handleRemoveFromEnrolled(student.id)}
            ...
            <div className="bg-white rounded-lg shadow border-l-4 border-l-amber-500">
              <div className="px-6 py-4 border-b border-gray-200 bg-amber-50">
                ...
                <p className="text-sm text-gray-600 mt-1">{program.waitlist.length} residents waiting</p>
              </div>
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {program.waitlist.map((resident, index) => (
                      <tr key={resident.id} className="hover:bg-amber-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                          {new Date(resident.requestedDate).toLocaleDateString()}
                        </td>
                        <td className="px-6 py-4 text-sm text-gray-500">
                          {resident.notes || <span className="text-gray-400 italic">No notes</span>}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm">
                          <div className="flex items-center gap-2">
                            <button
                              onClick={() => handleEnrollFromWaitlist(resident)}
                              disabled={enrollmentCount >= capacity}
                              ...
                              title={enrollmentCount >= capacity ? 'Program is at capacity' : 'Enroll in program'}
                            >
                            ...
                            <button
                              ...
                              disabled={index === program.waitlist.length - 1}
                              className={`p-2 rounded transition-colors ${
                                index === program.waitlist.length - 1
                                  ? 'text-gray-300 cursor-not-allowed'
                                  : 'text-blue-600 hover:text-blue-800 hover:bg-blue-50'
                              }`}
                              title="Move down"
                            ...
                            <button
                              onClick={() => handleRemoveFromWaitlist(resident.id)}
                              className="text-red-600 hover:text-red-800 hover:bg-red-50 p-2 rounded transition-colors"
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-lg shadow sticky top-8">
              <div className="px-6 py-4 border-b border-gray-200 bg-blue-50">
                <h2 className="text-xl font-semibold text-gray-900">Pending Kites</h2>
                <p className="text-sm text-gray-600 mt-1">{program.kites.length} requests awaiting review</p>
              </div>
              <div className="divide-y divide-gray-200 max-h-[calc(100vh-12rem)] overflow-y-auto">
                {program.kites.length === 0 ? (
                  <div className="px-6 py-8 text-center text-gray-500">
                    <p className="text-sm italic">No pending kites</p>
                    <p className="text-xs text-gray-400 mt-2">Kites will appear here when residents submit requests</p>
                  </div>
                ) : (
                  program.kites.map((kite) => {
                    const daysOld = getDaysOld(kite.timestamp);
                    const isOld = daysOld >= 7;
                    
                    return (
                      <div key={kite.id} className="px-6 py-4 hover:bg-gray-50">
                        ...
                        <div className="flex items-center gap-2 mb-3">
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                            ...
                            {kite.status === 'pending' ? 'Pending' : 'Confirmed'}
                          </span>
                          {isOld && (
                        ...
                        {/* Resident Info */}
                        <div className="mb-3">
                          <h3 className="font-semibold text-gray-900">{kite.name}</h3>
                          <p className="text-sm text-gray-600">ID: {kite.residentId}</p>
                          <p className="text-xs text-gray-500 mt-1">
                            Submitted: {new Date(kite.timestamp).toLocaleString()}
                          </p>
                        </div>
                        ...
                        {kite.notes && (
                          <div className="mb-4 p-3 bg-gray-50 rounded text-sm text-gray-700">
                            <p className="font-medium text-xs text-gray-500 mb-1">Note from resident:</p>
                            <p>{kite.notes}</p>
                          </div>
                        )}
                        ...
                        <div className="space-y-2">
                          {kite.status === 'pending' && (
                            <button
                              onClick={() => handleConfirmViewed(kite.id)}
